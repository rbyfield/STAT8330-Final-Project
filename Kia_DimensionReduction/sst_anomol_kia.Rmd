...
# precipitation over latitude for fixed long[mm/day]
plot(Y_prec,prec[100,,842], 
     type="l", xlab="Latitude", ylab="Precipiation [mm/day]",
     main="precipitation over latitude for fixed long[mm/day] at Jan 2018",
     lwd=3)
...
```{r}
EOFar=array(0, dim=c(12,6000,30)) #EOFar[month,gridID,modeNo]
...
var_eof=ncvar_def("EOFvalues", "dimensionless", 
                     ...
                     longname="EOF data from prec data 1948-2018: 
                               30modes 12months") 
...
```{r}
dat1=array(0,dim=c(120,50,30,12))
for (mo in 1:12){
  for (md in 1:30){
...
for (m in 1:12){
    n=1
    mapMatrix = - matrix(dat3[,,1,m]/sqrt(cos(LAT*pi/180)),nrow=120)
...
for (m in 1:12){
    n=2
    mapMatrix = - matrix(dat3[,,2,m]/sqrt(cos(LAT*pi/180)),nrow=120)
    mapMatrix = pmax(pmin(mapMatrix,0.05),-0.03)
    rgb.palette=colorRampPalette(c('red','yellow','orange', 'white', 
                                   'green','blue','darkblue'),interpolate='spline')
    int=seq(-0.03,0.05,length.out=61)
